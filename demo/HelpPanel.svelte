...
+アリス、ボブ、チャーリー</code></pre>
      <p>区切り文字はスペース・<code>,</code>・<code>、</code> などが使えます。参加行は自動的に先頭に移動されるため、文中のどこに書いても構いません。</p>
...
引き分け</code></pre>
    ...
    <section id="help-faq">
      ...
      <p>プレイヤー名は <code>+</code> 行で先に登録する必要があります。省略形は一意に特定できる場合のみマッチします。同じ前方一致になる名前が複数ある場合は、より多くの文字を入力してください。</p>
