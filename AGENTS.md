...
- `retar/` — Retar: analyzes Mafia/Werewolf village events to compute role possibilities.
...
## Conventions
- Keep dependencies isolated per project folder (avoid cross-imports between `howl/` and `retar/`).
...
- Retar analyzer: `retar/analyzer.ts`
- Retar village state/event model: `retar/village.ts`
- Retar bitset possibilities: `retar/analyzer/possibilities.ts`
