...
システム全体は「**現象が発生した日付** (= 行動日)」 を統一の Day 軸として共有する。 内部記録は常にこの軸で行い、 「報告日にずらした表示用の値」 を内部 state に持ち込まない。 表示だけ変える辻褄合わせ (= ごまかし) は混乱の温床なので禁止する。

### 時間軸の用語

時間順は `Day1 昼 → Day1 夜 → Day2 昼 → Day2 夜 → ...` で進む。
- 「Day N 夜」 = Day N の行動 (占い・襲撃・護衛等) が起きる夜
- 「Day N 昼」 = Day N の議論・投票・処刑が行われる昼

### ユーザー記法と行動日

役職者本人は自分の行動を**行動日**で語る — 占い師が `2日目 X ○` と書けば「**Day 2 の夜に X を占って ○ だった**」 という意味で、 N は行動日。 ユーザー記法 `N日目`、 parser 出力 `assertion.day`、 bridge 出力 `status.assertions` の key、 retar / hati / skoll の Day index 等は全て**行動日 N**で揃える。

`# Day N` dayMark は「**ゲーム上の現在地点** (= Day N)」 を示すだけで、 報告日ラベルでも翻訳境界でもない。 right-align の暗黙補完の文脈として使われる (下記)。

`Day 0` は通常存在しないが、 `role.seer.first-seek` 等の規定下では「Day 0 の夜」 だけが疑似的に存在する (= Day 1 昼の前夜 = ゲーム開始前夜の先制占い)。

### 観測・報告境界の例外

人間の直感「**昨日の行動の結果**」 に依存する 1 箇所だけが**例外**:

**Howl の right-align**: 日付ラベル無しの結果は「today-1 の行動結果」 (= ゲーム上の直前夜) と**暗黙に補完される**。 例えば `# Day 3` 文脈で `占いCO X ○` と書けば「行動 Day 2 の結果」 と解釈される。 これは役職者が明示的に日付を書かなかった場合の慣習補完であり、 ユーザーが日付ラベルを明示したとき (例: `2日目 X ○`) はそれが優先される。

### Status ペイン表示

行動日 N の結果は「Day N+1 の欄」 に表示される — Day 1 夜の行動結果の報告が行われるのは必ず翌日なので、 Day 2 の欄に Day 1 夜の結果を書き込む、 という表示慣習。 これは UI 慣習であり、 内部 state の Day 軸は行動日で一貫している。
