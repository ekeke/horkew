...
| モデル | 役職 | チームNW | 勝利条件 |
|--------|------|----------|----------|
| **村陣営** | villager, seer, medium, bodyguard, nekomata, mason | mason_team | villageWin |
| **狼陣営** | werewolf, fanatic | wolf_team | wolfWin |
| **第三陣営** | werehamster, immoralist | - | hamsterWin |

### 統合の根拠

**Seat Transformerが役職差異を吸収する**:
- `my_role(11)` がCLS tokenに入っている → Attentionで役職条件付き学習
- Private知識セクション（divine_result, wolf_teammate等）が役職ごとに異なるので入力レベルで差別化済み
- Action maskが不正行動を排除（占い先は占い師だけ、護衛先は狩人だけ）

**同一陣営は目的が同じ**:
- 村陣営: 人外排除。mason→村にマージ（共有者も「確定村として村を守る」が目的）
- 狼陣営: 狼勝利。fanatic→狼にマージ（狂信者も狼勝利で勝ち）
- 第三陣営: 狐勝利。immoralist→第三にマージ（背徳者も狐勝利で勝ち）

**実用面の利点**:
- 学習速度2倍（6並列→3並列、各モデルの学習データ密度が上がる）
- オーケストレーション単純化
- predict headが同一モデル内で全役職の推理を学ぶ → 推理精度向上

### チーム戦略

| チーム | 所属モデル | 追加入力 |
|--------|-----------|---------|
| wolf_team | 狼陣営 | +43次元（team_size, team_flags, fake_divine） |
| mason_team | 村陣営 | +43次元（team_size, team_flags） |

チーム戦略NWは所属モデルと**重みを共有しない**別ネットワーク（入力次元が異なるため）。
ただしSeat Transformerの構造は同一 → Step 2で重みコピー初期化が可能。

チーム追加入力の内訳（+43次元）:
```
team_size:          1    チーム人数
is_my_team:        14    各席がチームメンバーか (per-seat)
is_current_actor:  14    各席が現在の行動者か (per-seat)
fake_divine:       14    偽占い結果 (wolf_teamのみ使用、mason_teamは0埋め)
```

### 完全統一（1モデル）を採用しない理由

- **報酬の衝突**: 同じゲームで村+1/狼-1。共有trunkに矛盾した勾配が流れる
- **チーム入力の構造差**: wolf_teamは+43次元。この有無でネットワーク構造が変わる
- **情報隔壁の複雑化**: 1モデルで村と狼を同時に学ぶと、fake_hintsの遮断が複雑になる

将来的に戦略NNのtrunkだけ共有する（対戦相手モデリング）は検討可能だが、
まずは3モデル独立で実装し、ベースラインを確立する。
