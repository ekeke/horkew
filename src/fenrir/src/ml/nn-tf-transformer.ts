...
export class TfTransformerNetwork {
  ...
  /**
   * Outcome-weighted SL fine-tune (REINFORCE-like) with optional KL anchor.
   *
   *   advantage_i = outcomes[i] - baseline
   *   policyLoss  = -mean_i (advantage_i * logp(a_i | obs_i))
   *   klLoss      = mean_i KL(p_theta(obs_i) || p_ref(obs_i))   if refLogits && klCoeff > 0
   *   totalLoss   = policyLoss + klCoeff * klLoss
   *
   * headType dispatch (trainSupervisedHead と同形式):
   *   - perSeatSoftmax:  [n, SEATS] masked softmax、actionIndices が seat index
   *   - globalSoftmax:   [n, outputSize] softmax、actionIndices が index
   *   - perSeatSigmoid:  [n, SEATS*perDim] Bernoulli、actionMultiHot が multi-hot vector
   *
   * refLogits は Pure JS refNet.forward(obs).policies.get(headName) を渡す (生 logits)。
   * klCoeff=0 または refLogits 未指定なら KL 計算をスキップ。
   */
  trainOutcomeWeightedSL(batch: {
    observations: Float32Array[]
    outcomes: number[]
    baseline: number
    headName: string
    headType: 'perSeatSoftmax' | 'globalSoftmax' | 'perSeatSigmoid'
    /** softmax 系 head: 選んだ action の index */
    actionIndices?: number[]
    /** sigmoid head: 各 dim の 0/1 multi-hot vector */
    actionMultiHot?: Float32Array[]
    /** perSeatSoftmax のみ必須: [n, SEATS] 加算マスク (0 legal / -1e9 illegal) */
    masks?: Float32Array[]
    /** Pretrained net からの参照 logits。省略または klCoeff=0 で KL 無効 */
    refLogits?: Float32Array[]
    /** KL anchor 係数 (default 0 = 無効) */
    klCoeff?: number
  }): { loss: number, policyLoss: number, klLoss: number } {
    const n = batch.observations.length
    if (n === 0) return { loss: 0, policyLoss: 0, klLoss: 0 }
    ...
    const isSoftmax = batch.headType !== 'perSeatSigmoid'
    if (isSoftmax) {
      if (!batch.actionIndices || batch.actionIndices.length !== n) {
        throw new Error('trainOutcomeWeightedSL: actionIndices required for softmax head types')
      }
    } else {
      if (!batch.actionMultiHot || batch.actionMultiHot.length !== n) {
        throw new Error('trainOutcomeWeightedSL: actionMultiHot required for perSeatSigmoid head type')
      }
    }
    if (batch.headType === 'perSeatSoftmax') {
      if (!batch.masks || batch.masks.length !== n) {
        throw new Error('trainOutcomeWeightedSL: masks required for perSeatSoftmax head type')
      }
    }
    ...
    const advData = new Float32Array(n)
    for (let i = 0; i < n; i++) advData[i] = batch.outcomes[i] - batch.baseline

    let actionIdxData: Int32Array | null = null
    let actionMultiHotData: Float32Array | null = null
    if (isSoftmax) {
      actionIdxData = new Int32Array(n)
      for (let i = 0; i < n; i++) actionIdxData[i] = batch.actionIndices![i]
    } else {
      actionMultiHotData = new Float32Array(n * outputSize)
      for (let i = 0; i < n; i++) actionMultiHotData.set(batch.actionMultiHot![i], i * outputSize)
    }
    ...
    const useKL = klCoeff > 0 && !!batch.refLogits && batch.refLogits.length === n
    let refData: Float32Array | null = null
    if (useKL) {
      refData = new Float32Array(n * outputSize)
      for (let i = 0; i < n; i++) refData.set(batch.refLogits![i], i * outputSize)
    }

    const result = { loss: 0, policyLoss: 0, klLoss: 0 }
    ...
    const lossFunc = () => {
      ...
      if (batch.headType === 'perSeatSoftmax') {
        logits = this.perSeatLogits(seatOutputs, headW, headB)
      } else if (batch.headType === 'globalSoftmax') {
        logits = tf.add(tf.matMul(clsOut, headW), headB) as tf.Tensor2D
      } else {
        logits = this.perSeatSigmoidLogits(seatOutputs, headW, headB)
      }

      const advTensor = tf.tensor1d(advData)
      const eps = tf.scalar(1e-8)

      let policyLoss: tf.Scalar
      if (batch.headType === 'perSeatSigmoid') {
        const actionTensor = tf.tensor2d(actionMultiHotData!, [n, outputSize])
        const probs = tf.sigmoid(logits)
        const logP = tf.log(tf.add(probs, eps))
        const logOneMinusP = tf.log(tf.add(tf.sub(tf.scalar(1), probs), eps))
        const logpPerDim = tf.add(
          tf.mul(actionTensor, logP),
          tf.mul(tf.sub(tf.scalar(1), actionTensor), logOneMinusP),
        )
        const logpPerSample = tf.sum(logpPerDim, 1) as tf.Tensor1D
        policyLoss = tf.neg(tf.mean(tf.mul(advTensor, logpPerSample))) as tf.Scalar
      } else {
        let maskedLogits: tf.Tensor2D = logits
        if (maskData) {
          const maskTensor = tf.tensor2d(maskData, [n, outputSize])
          maskedLogits = tf.add(logits, maskTensor) as tf.Tensor2D
        }
        const probs = tf.softmax(maskedLogits)
        const logProbs = tf.log(tf.add(probs, eps))
        const idxTensor = tf.tensor1d(actionIdxData!, 'int32')
        const oneHot = tf.oneHot(idxTensor, outputSize)
        const logpChosen = tf.sum(tf.mul(logProbs, oneHot), 1) as tf.Tensor1D
        policyLoss = tf.neg(tf.mean(tf.mul(advTensor, logpChosen))) as tf.Scalar
      }

      let klLoss: tf.Scalar = tf.scalar(0) as tf.Scalar
      if (useKL && refData) {
        const refTensor = tf.tensor2d(refData, [n, outputSize])
        if (batch.headType === 'perSeatSigmoid') {
          const p = tf.sigmoid(logits)
          const q = tf.sigmoid(refTensor)
          const klPerDim = tf.add(
            tf.mul(p, tf.sub(tf.log(tf.add(p, eps)), tf.log(tf.add(q, eps)))),
            tf.mul(
              tf.sub(tf.scalar(1), p),
              tf.sub(
                tf.log(tf.add(tf.sub(tf.scalar(1), p), eps)),
                tf.log(tf.add(tf.sub(tf.scalar(1), q), eps)),
              ),
            ),
          )
          klLoss = tf.mean(tf.sum(klPerDim, 1)) as tf.Scalar
        } else {
          let curLogits: tf.Tensor2D = logits
          let refLogitsMasked: tf.Tensor2D = refTensor
          if (maskData) {
            const maskTensor = tf.tensor2d(maskData, [n, outputSize])
            curLogits = tf.add(logits, maskTensor) as tf.Tensor2D
            refLogitsMasked = tf.add(refTensor, maskTensor) as tf.Tensor2D
          }
          const p = tf.softmax(curLogits)
          const q = tf.softmax(refLogitsMasked)
          const klPerSample = tf.sum(
            tf.mul(p, tf.sub(tf.log(tf.add(p, eps)), tf.log(tf.add(q, eps)))),
            1,
          )
          klLoss = tf.mean(klPerSample) as tf.Scalar
        }
      }

      const totalLoss = tf.add(policyLoss, tf.mul(tf.scalar(klCoeff), klLoss)) as tf.Scalar

      result.policyLoss = policyLoss.dataSync()[0]
      result.klLoss = klLoss.dataSync()[0]
      result.loss = totalLoss.dataSync()[0]
