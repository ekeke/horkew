/**
 * Wolf perspective skoll 教師あり学習。
 *
 * mason 版 (trainer.ts) の wolf 版。
 *   - createWolfBrainTfNetwork で学習
 *   - createWolfBrainNetwork で eval
 *   - input サイズ = WOLF_COLLECTIVE_OBSERVATION_SIZE
 *   - vote head は wolf brain にも存在 (formation 等は学習対象外)
 */

import { mkdirSync, readFileSync } from 'node:fs'
import { dirname } from 'node:path'

import { createWolfBrainNetwork, createWolfBrainTfNetwork } from '../training.ts'
...
export type WolfTrainerOptions = {
  inputJsonl: string
  ...
  patience: number
  seed: number
  /** rawWolfWinRates から再計算 */
  relabelTemperature?: number
}

export const DEFAULT_WOLF_TRAINER_OPTIONS: WolfTrainerOptions = {
  inputJsonl: 'tmp/skoll-wolf-data/samples.jsonl',
  outputCheckpointPath: 'tmp/skoll-wolf-trainer/ckpt-wolf_brain/wolf_brain_final.json',
  batchSize: 256,
  miniBatchSize: 64,
  epochs: 30,
  learningRate: 3e-4,
...
function loadJsonl(path: string, relabelTemperature?: number): LoadedSample[] {
  ...
  for (const line of lines) {
    ...
    if (relabelTemperature !== undefined) {
      const rawWolfWinRates = obj.metadata?.rawWolfWinRates as Array<{ seat: number, wolfWinRate: number }> | undefined
      if (rawWolfWinRates && rawWolfWinRates.length > 0) {
        const candidates = rawWolfWinRates.map(r => ({ seat: r.seat, wolfWinRate: r.wolfWinRate, isTeammate: false }))
        const built = makeWolfSoftLabel(candidates, relabelTemperature)
        label = built.label
        mask = built.mask
      } else {
...
export async function trainAndSaveWolf(opts: Partial<WolfTrainerOptions> = {}): Promise<{
  numTrain: number
  numEval: number
  bestEpoch: number
  bestEvalLoss: number
  ...
  const options = { ...DEFAULT_WOLF_TRAINER_OPTIONS, ...opts }
  process.stderr.write(`[wolf-train] loading ${options.inputJsonl}\n`)
  if (options.relabelTemperature !== undefined) {
    process.stderr.write(`[wolf-train] relabel-temperature=${options.relabelTemperature}\n`)
  }
  const all = loadJsonl(options.inputJsonl, options.relabelTemperature)
  process.stderr.write(`[wolf-train] loaded ${all.length} samples\n`)
  ...
  process.stderr.write(`[wolf-train] split: train=${trainSet.length}, eval=${numEval}\n`)

  const tfNet = createWolfBrainTfNetwork(options.learningRate)
  const evalNet = createWolfBrainNetwork()
  ...
  for (let epoch = 0; epoch < options.epochs; epoch++) {
    ...
    process.stderr.write(
      `[wolf-train] epoch ${epoch + 1}/${options.epochs} `
    ...
    if (evalRes.loss < bestEvalLoss) {
      ...
      if (patienceCounter >= options.patience) {
        process.stderr.write(`[wolf-train] early stop at epoch ${epoch + 1} (patience ${options.patience})\n`)
        break
      }
    }
  }
  ...
  process.stderr.write(`[wolf-train] === Done ===\n`)
  process.stderr.write(`[wolf-train] best epoch: ${bestEpoch + 1}\n`)
  process.stderr.write(`[wolf-train] best eval: loss=${bestEvalLoss.toFixed(4)} top1=${bestEvalTop1.toFixed(3)} top3=${bestEvalTop3.toFixed(3)}\n`)
  process.stderr.write(`[wolf-train] checkpoint → ${options.outputCheckpointPath}\n`)
...
function parseCli(): Partial<WolfTrainerOptions> {
  const opts: Partial<WolfTrainerOptions> = {}
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--help':
        process.stderr.write('Usage: wolf-trainer.ts [--input PATH] [--output PATH] [--batch N] [--epochs N] [--lr R] [--eval-ratio R] [--patience N] [--seed S] [--relabel-temperature T]\n')
        process.exit(0)
    }
  }
  return opts
...
if (process.argv[1]?.endsWith('wolf-trainer.ts')) {
  trainAndSaveWolf(parseCli()).catch(err => {
    console.error(err)
    process.exit(1)
  })
}
