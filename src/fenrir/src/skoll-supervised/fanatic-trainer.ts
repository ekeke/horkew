/**
 * Fanatic perspective skoll 教師あり学習 (standard NN)。
 */
...
export type FanaticTrainerOptions = {
  inputJsonl: string
...
export const DEFAULT_FANATIC_TRAINER_OPTIONS: FanaticTrainerOptions = {
  inputJsonl: 'tmp/skoll-fanatic-data/samples.jsonl',
  outputCheckpointPath: 'tmp/skoll-fanatic-trainer/ckpt-fanatic/fanatic_final.json',
  batchSize: 256,
  epochs: 30,
  learningRate: 3e-4,
  evalRatio: 0.1,
...
function loadJsonl(path: string, relabelTemperature?: number): LoadedSample[] {
  ...
  for (const line of lines) {
    ...
    if (relabelTemperature !== undefined) {
      const raws = obj.metadata?.rawWolfWinRates as Array<{ seat: number, wolfWinRate: number }> | undefined
      if (raws && raws.length > 0) {
        const candidates = raws.map(r => ({ seat: r.seat, wolfWinRate: r.wolfWinRate, isTeammate: false }))
...
export async function trainAndSaveFanatic(opts: Partial<FanaticTrainerOptions> = {}) {
  const options = { ...DEFAULT_FANATIC_TRAINER_OPTIONS, ...opts }
  process.stderr.write(`[fanatic-train] loading ${options.inputJsonl}\n`)
  const all = loadJsonl(options.inputJsonl, options.relabelTemperature)
  process.stderr.write(`[fanatic-train] loaded ${all.length} samples\n`)
  ...
  process.stderr.write(`[fanatic-train] split: train=${trainSet.length}, eval=${numEval}\n`)
  ...
  for (let epoch = 0; epoch < options.epochs; epoch++) {
    ...
    process.stderr.write(`[fanatic-train] epoch ${epoch + 1}/${options.epochs} train: loss=${trainLoss.toFixed(4)} acc=${trainAcc.toFixed(3)} | eval: loss=${evalRes.loss.toFixed(4)} top1=${evalRes.top1.toFixed(3)} top3=${evalRes.top3.toFixed(3)}\n`)
    ...
    if (evalRes.loss < bestEvalLoss) {
      ...
      process.stderr.write(`[fanatic-train] early stop at epoch ${epoch + 1} (patience ${options.patience})\n`)
      break
    }
  }
  tfNet.dispose()

  process.stderr.write(`[fanatic-train] === Done === best epoch ${bestEpoch + 1}, loss=${bestEvalLoss.toFixed(4)} top1=${bestEvalTop1.toFixed(3)} top3=${bestEvalTop3.toFixed(3)}\n`)
...
if (process.argv[1]?.endsWith('fanatic-trainer.ts')) {
  trainAndSaveFanatic({}).catch(err => { console.error(err); process.exit(1) })
}
