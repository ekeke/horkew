                  ...
                  <span class="va-poss-item">
                    <span class="va-poss-row">
                      <PlayerName
                        ...
                        seat={member.seat}
                        status={memberStatus}
                      >{ctx.playerShortNames.get(member.seat) ?? ctx.players.get(member.seat) ?? `#${member.seat}`}</PlayerName>{#if isClaiming}{#each possibilities as { role, dim }}{@const assumed = ctx.mergedAssumptions.get(member.seat) === role}{@const assumedBySpoiler = assumed && ctx.spoilerAssumptions.has(member.seat)}{@const confirmed = !assumed && memberConfirmedRole === role}{@const confirmedAlign = confirmed ? systemRoles.get(role)?.alignment : undefined}<span class="va-poss-cell" class:role-possible={!assumed && !confirmed && !dim} class:role-dim={!assumed && !confirmed && dim} class:role-assumed={assumed} class:role-assumed-spoiler={assumedBySpoiler} class:role-confirmed={confirmed} class:confirmed-village={confirmed && confirmedAlign === 'villager'} class:confirmed-wolf={confirmed && confirmedAlign === 'werewolf'} class:confirmed-fox={confirmed && confirmedAlign === 'werehamster'} onclick={() => ctx.toggleAssumption(member.seat, role)} role="button" tabindex="0">{roleToShort(role)}</span>{/each}{/if}{#if shouldShowHocuspocus(member.seat)}<span class="va-hocuspocus-cell" class:hocuspocus-on={ctx.hocusPocusSeats.has(member.seat)} title="HocusPocus: この席のCOを無視して解析" onclick={() => ctx.toggleHocusPocus(member.seat)} role="button" tabindex="0">?</span>{/if}
                    </span>
                  </span>
                {/each}
