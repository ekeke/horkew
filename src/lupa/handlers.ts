...
/** 投票フェーズコンテキスト */
export type VoteContext = PhaseContext & {
  /** 再投票ラウンド (0=初回) */
  revoteRound: number
  /** 再投票候補 (null=全員が対象) */
...
export type GameHandlers = {
  ...
  onNight(ctx: PhaseContext): MaybePromise<Map<number, NightAction>>

  /** 昼COフェーズ: 各プレイヤーのCO/結果報告を返す */
  onDayClaims(ctx: PhaseContext): MaybePromise<Map<number, DayClaim>>
