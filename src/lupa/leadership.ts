 ...
 * 2. 占い師確定: 占いCO者が1人のみ + 対抗なし
 */
export function detectCommander(state: GameState): number | null {
  const alive = alivePlayers(state)

  // 1. 共有者ペア確認
  const masonClaimers = alive.filter(p => p.claimedRole === 'mason')
  for (const a of masonClaimers) {
    for (const b of masonClaimers) {
      if (a.seat >= b.seat) continue
      // AがBを相方と宣言 && BがAを相方と宣言
      // masonCO時のpartner情報はGameEventに記録されている
      ...
      if (aPartner === b.seat && bPartner === a.seat) {
        return a.seat // seat低い方
      }
    }
  }

  // 2. 占い師確定: 生存占いCO者が1人のみ
